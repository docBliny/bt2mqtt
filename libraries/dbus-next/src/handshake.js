...
function getUid() {
  if ("DBUS_UID" in process.env) return Number(process.env.DBUS_UID);
  if ("getuid" in process) return process.getuid();
  return null;
}
...
function tryAuth(stream, methods, cb) {
  ...
  switch (authMethod) {
    ...
    case "DBUS_COOKIE_SHA1":
      stream.write(`AUTH ${authMethod} ${id}\r\n`);
      readLine(stream, function (line) {
        const data = Buffer.from(line.toString().split(" ")[1].trim(), "hex").toString().split(" ");
