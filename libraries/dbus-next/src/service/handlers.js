...
const { isObjectPathValid, isInterfaceNameValid, isMemberNameValid } = require("../validators");

const { ACCESS_READ, ACCESS_WRITE, ACCESS_READWRITE } = require("./interface");

const constants = require("../constants");

const { METHOD_RETURN } = constants.MessageType;
  ...
  if (value.signature !== options.signature) {
    bus.send(
      Message.newError(
        msg,
        INVALID_ARGS,
        `Cannot set property '${prop}' with signature '${value.signature}' (expected '${options.signature}')`,
      ),
    );
    return;
  }
      ...
      const sendReply = (body) => {
        ...
        if (method.outSignatureTree.length !== body.length) {
          return sendServiceError(
            bus,
            msg,
            `method ${iface.$name}.${m} returned the wrong number of arguments (got ${body.length} expected ${method.outSignatureTree.length}) for signature '${method.outSignature}'`,
          );
        }
