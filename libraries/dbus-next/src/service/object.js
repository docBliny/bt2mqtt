  ...
  addInterface(iface) {
    ...
    const signalHandler = function (options, result) {
      ...
      if (signatureTree.length !== result.length) {
        throw new Error(
          `signal ${iface.$name}.${name} returned the wrong number of arguments (got ${result.length} expected ${signatureTree.length}) for signature '${signature}'`,
        );
      }
    ...
    return [
      ...
      },
      {
        ...
        method: [
          ...
          {
            $: { name: "GetAll" },
            arg: [{ $: { direction: "in", type: "s" } }, { $: { direction: "out", type: "a{sv}" } }],
          },
        ],
        signal: [
          {
